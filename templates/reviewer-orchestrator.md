...
**Note (GH#55, GH#94):** when this `result=sub-agent-failed` line is emitted, the wrapper (`runners/run-reviewer.sh`) post-processes the log and posts a stub `[reviewer-agent: blocked]` review on `pr=#<PR>` so `eligibility_review_pending` does not re-fire the orchestrator + sub-agent on the same head SHA next cycle. After `REVIEWER_SUB_AGENT_FAILURE_CAP` consecutive stub reviews on the same PR (default 3) the wrapper escalates by applying `REVIEWER_ESCALATION_LABEL` and posting one explanation comment instead of yet another stub — closing the per-PR loop on deterministically-failing PRs (oversized diff, malformed PR) where every new push would otherwise reproduce the same failure and accumulate stubs indefinitely. The wrapper's stub-counter and the GH#94 cap both grep for the same marker substring (`Sub-agent run failed before posting a review`), so if you ever change the failure-marker format above, update **both** the wrapper's grep and the GH#94 cap-counter at the same time. The orchestrator's hard rule below forbids the orchestrator itself from posting that review, which is why the fallback lives in the wrapper.
