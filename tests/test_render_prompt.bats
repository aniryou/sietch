...
setup() {
  RIG=$(make_rig)
  export REPO_ROOT="$RIG"
  export SIETCH_HOME="$SIETCH_ROOT"
  TEMPLATE="$SIETCH_ROOT/tests/fixtures/template-all-keys.md"
}

@test "every RIG_KEYS variable substitutes (no literal \${KEY} left for any rig key)" {
  run bash "$SIETCH_ROOT/lib/render-prompt.sh" "$TEMPLATE"
  [ "$status" -eq 0 ]
  # No literal \${KEY} remaining for any of the known rig keys.
...
@test "concrete rig values appear in output" {
  run bash "$SIETCH_ROOT/lib/render-prompt.sh" "$TEMPLATE"
  [ "$status" -eq 0 ]
  ...
  # The fix to sietch-2lp ensures REVIEWER_AGENT_VERDICT_REGEX renders. Note
  # the rig.config value contains literal backslashes (\[ and \]) so the
...
@test "non-RIG_KEYS variables pass through literally" {
  run bash "$SIETCH_ROOT/lib/render-prompt.sh" "$TEMPLATE"
  [ "$status" -eq 0 ]
  # \${REPO_ROOT} and \${HOLDER} are NOT in RIG_KEYS — must remain literal so
...
@test "missing template path exits 2" {
  run bash "$SIETCH_ROOT/lib/render-prompt.sh" "$BATS_TEST_TMPDIR/does-not-exist.md"
  [ "$status" -eq 2 ]
  echo "$output" | grep -qF '[render-prompt] template not found'
}

@test "missing rig.config exits 1" {
  REPO_ROOT="$BATS_TEST_TMPDIR/empty-rig" \
    run bash "$SIETCH_ROOT/lib/render-prompt.sh" "$TEMPLATE"
  [ "$status" -eq 1 ]
...
@test "envsubst not on PATH exits 1 with helpful message" {
  ...
  PATH=/usr/bin:/bin run bash "$SIETCH_ROOT/lib/render-prompt.sh" "$TEMPLATE"
