...
# `"${empty_array[@]}"`. Empty when unset (interactive `st review`).
_dispatch_kv=()
...
event_emit reviewer llm_started mode=default pr="$TARGET_PR" "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"
_llm_start_s=$(date +%s)
set -m
(
...
event_emit reviewer llm_exited mode=default pr="$TARGET_PR" exit_code="$LLM_EXIT" duration_s="$(($(date +%s) - _llm_start_s))" "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}" "${_llm_cost_kv[@]}"
...
if [ -f "$RAW" ]; then
  ...
  if [ "$BASH_CALL_COUNT" -gt "$REVIEWER_BASH_CALL_GUIDANCE" ]; then
    ...
    event_emit reviewer bash_overshoot count="$BASH_CALL_COUNT" guidance="$REVIEWER_BASH_CALL_GUIDANCE" pr="$TARGET_PR" "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"
  fi
...
if [ "$LLM_EXIT" -eq 0 ]; then
  ...
  if [ "$HAS_RESULT_LINE" -eq 0 ]; then
    ...
    event_emit reviewer hard_failure mode=default pr="$FAILED_PR" reason=no-result-line "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"
    ...
    if [ -n "$PR_DRIFT_DATA" ]; then
      ...
      if [ "$DRIFT_VERDICT" = "drift" ]; then
        ...
        event_emit reviewer verdict_drift pr="$TARGET_PR" "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"
