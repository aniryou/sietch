...
# `llm_exited` / `hard_failure` chain. Expanded at each event_emit call as
# `"${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"` — the `:+` form short-
# circuits on empty arrays so we don't trip bash 3.2's set-u rule against
# `"${empty_array[@]}"`. Empty when unset (interactive `st dev`, or
# loop_dev_mode1's foreground call which is not a dispatch).
_dispatch_kv=()
...
event_emit dev llm_started mode="$MODE" run_id="$DEV_AGENT_RUN_ID" "${_llm_target_kv[@]}" "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"
_llm_start_s=$(date +%s)
set -m
(
...
event_emit dev llm_exited mode="$MODE" exit_code="$LLM_EXIT" duration_s="$(($(date +%s) - _llm_start_s))" "${_llm_target_kv[@]}" "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}" "${_llm_cost_kv[@]}"
...
if [ "$MODE" = "default" ] && [ -n "${DEV_AGENT_TARGET_ISSUE:-}" ]; then
  ...
  if [ "$LLM_EXIT" -ne 0 ]; then
    ...
    event_emit dev hard_failure mode="$MODE" issue="$DEV_AGENT_TARGET_ISSUE" exit_code="$LLM_EXIT" retry_count="$_retry_next" reason="$_hf_reason" "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"
    unset _hf_reason
...
EOF
  ...
  event_emit dev hard_failure mode="$MODE" pr="$TARGET_PR" exit_code="$LLM_EXIT" reason=mode3-give-up "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"
...
if [ "$MODE" = "follow-up" ] && [ "$LLM_EXIT" -ne 0 ]; then
  ...
  # reach this branch.
  event_emit dev hard_failure mode="$MODE" pr="$TARGET_PR" exit_code="$LLM_EXIT" reason=mode2-give-up "${_dispatch_kv[@]:+"${_dispatch_kv[@]}"}"
