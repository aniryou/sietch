...
| 2 | Mode 2 dev-agent (followup)  | dispatcher    | `$DISPATCH_LOCK_DIR`    | `${LOCK_NAME_PREFIX}pr-N-followup.lock`               | `runners/run-loop.sh:268` (`mkdir`)             | `runners/run-loop.sh:78-100` (`cleanup_stale_dispatch_locks`, `kill -0` GC) + `runners/run-loop.sh:705-712` (session stop) | Single `dispatch:followup` tmux pane per repo (`runners/run-loop.sh:647,659`); no parallel dispatcher exists. The `mkdir` is defense-in-depth against re-entry within the same pane (a wrapper still running from the previous cycle). Lock is released by PID-based GC on the next cycle once the wrapper exits. |
| 3 | Mode 3 dev-agent (conflicts) | dispatcher    | `$DISPATCH_LOCK_DIR`    | `${LOCK_NAME_PREFIX}pr-N-conflicts.lock`              | `runners/run-loop.sh:411` (`mkdir`)             | `runners/run-loop.sh:78-100` (`cleanup_stale_dispatch_locks`) + `runners/run-loop.sh:705-712` (session stop) | Single `dispatch:conflicts` tmux pane per repo (`runners/run-loop.sh:648,660`). Same shape as row 2. PR-N-followup and PR-N-conflicts are disjoint lock names so the same PR may carry both lock dirs simultaneously without collision. |
...
| `$DISPATCH_LOCK_DIR` (rows 2, 3) | session stop only (`runners/run-loop.sh:705-712`) | **none** — same situation as `$LOCK_DIR` | yes — `cleanup_stale_dispatch_locks` runs `kill -0 $(cat lock/pid)` each dispatcher cycle (`runners/run-loop.sh:78-100`) | yes — PID GC catches it on the next cycle |
...
- **Mode 2 and Mode 3 share `$DISPATCH_LOCK_DIR`** but use distinct lock
  ...
  budget (`runners/run-loop.sh:108-118`), which is intentional —
...
- **TOCTOU window between dispatcher `mkdir` and child PID write** is
  ...
  spawning the child (`runners/run-loop.sh:269, 412`), then overwriting
  it with the child PID after `&` (`runners/run-loop.sh:274, 417`). The
...
`cleanup_stale_dispatch_locks` (`runners/run-loop.sh:78-100`) provides
